...
def readSignal():
    global poti1
    with VSSClient('192.168.0.1', 55555) as client:

        for updates in client.subscribe_current_values([
            'Vehicle.Speed',
            # 'Vehicle.Powertrain.PowerOptimizeLevel',
        ]):
            poti1 = int(updates['Vehicle.Speed'].value)
            # print(f"Received updated poti1: {poti1}")

t1 = threading.Thread(target=readSignal)
...
while (count>0):
    ...
    while True: # the game loop
        ...
        car_pos = poti1 / 255 * WINDOWWIDTH
        print('car_pos: ', car_pos)
        playerRect.x = car_pos
        # playerRect.move_ip(car_pos, 0)
